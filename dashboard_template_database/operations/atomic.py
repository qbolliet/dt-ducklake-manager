...
class AtomicDatabaseOperations:
    ...
    def execute_batch_update(self, 
        ...
        try:
            self.logger.info(f"Starting atomic batch update with {len(update_data)} rows")
            
            # Pré-validation si activée
            if config.enable_validation:
                validation_report = self.auditor.validate_operation_preconditions('update', df=update_data)
            ...
            if not validation_passed:
                result.recommendations.append("Review validation issues that were not critical")
            
            self.logger.info(f"Batch update completed successfully in {execution_time:.2f}s")
            return result
    ...
    def execute_schema_migration(self,
        ...
        try:
            self.logger.info(f"Starting atomic schema migration with {len(migration_operations)} operations")
            ...
            try:
                ...
                for i, migration_op in enumerate(migration_operations):
                    ...
                    if not self.transaction_mgr.execute_operation(tx_id):
                        raise Exception(f"Failed to execute migration operation {i+1}")
                    
                    operations_performed.append(f"Migration operation {i+1} completed")
                ...
                result = AtomicOperationResult(
                    ...
                    metrics={
                        'migration_operations_count': len(migration_operations)
                    }
                )
                
                self.logger.info(f"Schema migration completed successfully in {execution_time:.2f}s")
                return result
    ...
    def execute_cleanup_operation(self,
        ...
        try:
            ...
            if 'filters' in cleanup_config:
                ...
                if rows_deleted >= 0:
                    operations_performed.append(f"Deleted {rows_deleted} rows based on filters")
            ...
            if 'columns_to_remove' in cleanup_config:
                ...
                operations_performed.append(f"Deleted {successful_deletions}/{len(columns)} columns")
            ...
            for category, result in cleanup_results.items():
                if result:
                    if isinstance(result, dict):
                        ...
                        operations_performed.append(f"Cleaned {category}: {total} items")
                    elif isinstance(result, list):
                        operations_performed.append(f"Cleaned {category}: {len(result)} items")
                    else:
            ...
            result = AtomicOperationResult(
                ...
                operations_performed=operations_performed,
                metrics={
                    'cleanup_categories': len(cleanup_results)
                }
            )
            
            self.logger.info(f"Cleanup operation completed successfully in {execution_time:.2f}s")
            return result
    ...
    def execute_validation_and_repair(self,
        ...
        try:
            self.logger.info(f"Starting atomic validation and repair at level: {validation_level.value}")
            ...
            operations_performed = []
            operations_performed.append(f"Initial validation found {initial_issues} issues ({critical_issues} critical)")
            ...
            if recovery_result.success:
                ...
                operations_performed.append(f"Final validation: {final_issues} issues ({final_critical} critical)")
                ...
                if final_critical > 0:
                    result.recommendations.append("Manual intervention required for remaining critical issues")
                
                self.logger.info(f"Validation and repair completed in {execution_time:.2f}s")
                return result
            else:
                return self._create_failed_result(
                    config.operation_type,
    ...
    def execute_backup_and_update(self,
        ...
        try:
            self.logger.info(f"Starting atomic backup and update with {len(update_data)} rows")
            ...
            if not update_result.success:
                ...
                recovery_result = self.recovery_mgr.recover_database(recovery_op, confirm_destructive=True)
            ...
            result.recommendations.append("Backup created and can be used for future recovery")
            
            self.logger.info(f"Backup and update completed successfully in {execution_time:.2f}s")
            return result
    ...
    def validate_system_integrity(self) -> Dict[str, Any]:
        ...
        try:
            ...
            components_status = {
                'transaction_manager': len(self.transaction_mgr.list_active_transactions()) >= 0,
    ...
    def emergency_recovery(self, 
                         ...
                         allow_destructive: bool = False) -> AtomicOperationResult:
        """
        ...
        try:
            ...
            if recovery_strategy is None:
                if critical_issues > 5:  # Beaucoup d'issues critiques
                    recent_backups = [p for p in self.recovery_mgr.list_recovery_points() 
                                    if time.time() - p.timestamp < 86400]  # Moins de 24h
            ...
            recovery_result = self.recovery_mgr.recover_database(recovery_op, confirm_destructive=allow_destructive)
            ...
            if recovery_result.success:
                result = AtomicOperationResult(
                    ...
                    validation_passed=recovery_result.validation_report.get_critical_issues_count() == 0 if recovery_result.validation_report else True,
                    recommendations=recovery_result.recommendations
                )
                
                self.logger.info(f"Emergency recovery completed successfully in {execution_time:.2f}s")
                return result
            else:
                return AtomicOperationResult(
                    success=False,
    ...
    def _create_failed_result(self, 
                            ...
                            error_message: str,
                            backup_created: Optional[str] = None,
                            rollback_performed: bool = False) -> AtomicOperationResult:
    ...
    def _rollback_to_backup(self, backup_id: str) -> bool:
        ...
        try:
            ...
            result = self.recovery_mgr.recover_database(recovery_op, confirm_destructive=True)
