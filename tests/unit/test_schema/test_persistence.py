...
def test_duplicate_check_uses_primary_keys(sample_df):
    ...
    id_unique = builder.conn.execute("SELECT COUNT(DISTINCT id) FROM fact_table").fetchone()[0]
...
@pytest.mark.xfail(reason="PARTITION BY non supporté par la version courante de l'extension DuckLake", strict=False)
